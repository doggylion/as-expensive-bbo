...
def preprocessing_table2(table_data_file_path1, table_data_file_path2, dim, cross_valid_type, ap_columns, problem_info_columns):
    table_df1 = pd.read_csv(table_data_file_path1, header=0)
    table_df2 = pd.read_csv(table_data_file_path2, header=0)

    # print(table_df1, table_df2)

    table_df = pd.concat([table_df1, table_df2.iloc[:, 33:].add_prefix('arg_')], axis=1)
    # All dimensions are considered in LOPOAD-CV
    if cross_valid_type == 'lopo_cv' or cross_valid_type == 'loio_cv':
        table_df = table_df[table_df['dim'] == dim]
...
def hiearchical_regression_lopo_cv(as_result_dir_path, table_data_file_path1, table_data_file_path2, dim, left_fun_id, ml_seed, cross_valid_type, problem_info_columns, ap_columns, ap_algs, feature_selector, n_features_to_select, option, dir_sampling_method):
    if table_data_file_path2 == None:
        table_df = preprocessing_table(table_data_file_path1, dim, cross_valid_type, ap_columns, problem_info_columns)
    else:
        table_df = preprocessing_table2(table_data_file_path1, table_data_file_path2, dim, cross_valid_type, ap_columns, problem_info_columns)
    ...
    for alg in ap_algs:
        ...
        model_dict[alg].fit(train_df, y_train_dict[alg])
        pred_relert_dict[alg] = estimator.predict(test_df)

        print(pred_relert_dict[alg])
    ...
    for alg in ap_algs:
        ...
        importances = model_dict[alg].feature_importances_

        arg = np.argsort(importances)[::-1]

        arg_series = pd.Series(arg)
        labels_series = pd.Series(labels)
        
        df = pd.DataFrame({'arg':arg_series, 'labels':labels_series})

        file_path = os.path.join('feature_impotance', option, 'DIM{}/f{}/{}'.format(dim, left_fun_id, alg))
        os.makedirs(file_path, exist_ok=True)
        path = os.path.join(file_path, '{}.csv'.format(dir_sampling_method))
        df.to_csv(path)

        # legend = []
        # importance_arg = []
        # for i in range(10):
        #     legend.append(labels[arg[i]])
        #     importance_arg.append(importances[arg[i]])

        # print(type(importances))

        # plt.figure(figsize = (10,6))
        # plt.barh(y = range(len(importance_arg)), width = importance_arg)
        # plt.yticks(ticks = range(len(legend)), labels = legend)
        # file_path = os.path.join('feature_impotance', option, '{}_f{}_DIM{}_ix{}.pdf'.format(dir_sampling_method, left_fun_id, dim, instance_id))
        # plt.tight_layout()
        # plt.savefig(file_path)

        # print(r'%%%%%%%%%%%')
        # print(r'\subfloat[$s^{\prime}='+'{}'.format(option)+r'$]{')
        ...
        # print(r'feature & impotance\\')

        # print(r'\midrule')

        # for feature, imp in zip(legend, importance_arg):
        #     print('{}'.format(feature.replace('_', '-'))+r' & $'+'{:,.4f}'.format(imp)+r'$\\')
...
@click.option('--dir_sampling_method', '-dsample', required=False, default='lhs_multiplier50_sid0', type=str, help='Directory of a sampling method.')
...
@click.option('--per_metric', '-pm', required=False, default='RMSE', type=str, help='A performance metric.')
...
def main(ap_name, dir_sampling_method, as_run_id, ela_feature_classes, dims, selector, cross_valid_type, dim, left_dim, left_fun_id, left_instance_id, feature_selector, n_features_to_select, per_metric, surrogate_multiplier, surrogate_sample, comp_option, separation):
    np.random.seed(seed=as_run_id)
    random.seed(as_run_id)
    # ml_seed determines the seed of random state in a sklearn model
    ml_seed = as_run_id

    os.makedirs('feature_impotance', exist_ok=True)    
    ...
    if comp_option == 'comp':
        ...
        sample_dir_path = os.path.join('./sample_data/0', dir_sampling_method)
        table_data_file_path = os.path.join(ap_dir_path, 'feature_table_data/0/{}.csv'.format(table_data_name))
        table_data_file_path2 = os.path.join(ap_dir_path, 'feature_table_data/{}/{}.csv'.format(option, table_data_name))
        as_result_dir_path = os.path.join('as_results/0and{}/{}_{}_{}_{}_{}'.format(option, ap_name, selector, cross_valid_type, table_data_name, feature_selector))
    elif surrogate_multiplier == 0:
        sample_dir_path = os.path.join('./sample_data/0', dir_sampling_method)
        table_data_file_path = os.path.join(ap_dir_path, 'feature_table_data/0/{}.csv'.format(table_data_name))
        as_result_dir_path = os.path.join('as_results/0/{}_{}_{}_{}_{}'.format(ap_name, selector, cross_valid_type, table_data_name, feature_selector))
        option = '0'
        table_data_file_path2 = None
    else:
        if separation == 'none':
            option = '{}n_{}'.format(surrogate_multiplier, surrogate_sample)
        else:
            option = '{}n_{}_{}'.format(surrogate_multiplier, surrogate_sample, separation)
        sample_dir_path = os.path.join('./sample_data/{}'.format(option), dir_sampling_method)
        table_data_file_path = os.path.join(ap_dir_path, 'feature_table_data/{}/{}.csv'.format(option, table_data_name))
    ...
    path = os.path.join('feature_impotance', option)
    ...
    if selector == 'multiclass_classification' and cross_valid_type == 'lopo_cv':
        ...
        hiearchical_regression_lopo_cv(as_result_dir_path, table_data_file_path, table_data_file_path2, dim, left_fun_id, ml_seed, cross_valid_type, problem_info_columns, ap_columns, ap_algs, feature_selector, n_features_to_select, option, dir_sampling_method)        
        ...
        clustering_lopoad_cv(as_result_dir_path, table_data_file_path, left_dim, left_fun_id, ml_seed, cross_valid_type, problem_info_columns, ap_columns, ap_algs, feature_selector, n_features_to_select)
