...
def portfolio_construction(portfolio_list, sampling):
    """
    ...
    # ap_dict = {
        ...
        # 'ls16':['LSstep', 'MCS', 'AVGNEWUOA', 'MOS', 'BIPOP-saACM-k', 'fmincon', 'MLSL', 'OQNLP', 'P-DCN', 'BrentSTEPqi', 'DTS-CMA-ES', 'PSA-CMA-ES', 'DTS-CMA-ES_005-2pop_v26_1model', 'HE-ES', 'lq-CMA-ES', 'SLSQP-11-scipy'],
    ...
    #     'ls2':pfpy.greedy(2, pp_dir_path),
    #     'ls4':pfpy.greedy(4, pp_dir_path),
    #     'ls6':pfpy.greedy(6, pp_dir_path),
    #     'ls8':pfpy.greedy(8, pp_dir_path),
    #     'ls10':pfpy.greedy(10, pp_dir_path),
    #     'ls12':pfpy.greedy(12, pp_dir_path),
    #     'ls14':pfpy.greedy(14, pp_dir_path),
    #     'ls16':pfpy.greedy(16, pp_dir_path),
    #     'ls18':pfpy.greedy(18, pp_dir_path)
    # }
    ap_dict = {}
    for sampling_multiplier in sampling:
        ap_dict['ls{}'.format(sampling_multiplier)] = pfpy.greedy(4, sampling_multiplier)
...
def pp_alg_portfolio_data(bbob_suite, dims, feval_time, fun_ids, sampling_multiplier, target='-2.0', per_metric='accuracy'):
    """
    ...
    ----------
    """    
    ap_dir_path = os.path.join('./alg_portfolio', 'ls{}'.format(sampling_multiplier))
    pp_dir_path = os.path.join('./pp_bbob_exdata/ranking_feval{}D'.format(feval_time))
    ap_config_file_path = os.path.join(ap_dir_path, 'ap_config.csv')
    ...
    # copy_dir_path = os.path.join(ap_dir_path, 'copyls{}'.format(sampling_multiplier))
    metric_dir_path = os.path.join(ap_dir_path, per_metric)
    # os.makedirs(copy_dir_path, exist_ok=True)
    os.makedirs(metric_dir_path, exist_ok=True)    
    for alg in ap_conifg_list:
        for dim in dims:
            for fun_id in fun_ids:
                metric_value = [0 for i in range(5)]
                
                # Only the first five instances are considered
                for instance_id in range(1, 5+1):                
                    original_file_path = './pp_{}_exdata/accuracy/sampling{}D/{}/f{}_DIM{}_i{}.csv'.format(bbob_suite, sampling_multiplier, alg, fun_id, dim, instance_id)

                    feval = 0
                    with open(original_file_path, 'r') as fh:
                        for str_line in fh:
                            if feval <= feval_time*dim:
                                feval, accuracy, RMSE = str_line.split(',')
                                feval = int(float(feval.replace('\n','')))
                                accuracy = float(accuracy.replace('\n',''))
                                RMSE = float(RMSE.replace('\n',''))
                            else:                            
                                # feval_copy_file_path = os.path.join(copy_dir_path, '{}_f{}_DIM{}_i{}.csv'.format(alg, fun_id, dim, instance_id))
                                # if not os.path.exists(feval_copy_file_path):
                                #     with open(feval_copy_file_path, 'w') as fh:
                                #         fh.write("{},{},{}".format(feval, accuracy, RMSE))
                                
                                if per_metric == 'accuracy':
                                    metric_value[instance_id-1] += accuracy
                                elif per_metric == 'RMSE':
                                    metric_value[instance_id-1] += RMSE
                                # metric_copy_file_path = os.path.join(metric_dir_path, '{}_f{}_DIM{}.csv'.format(alg, fun_id, dim))
                                # with open(metric_copy_file_path, 'w') as fh:
                                #     fh.write("{},{}\n".format(instance_id, metric_value))
                                break
    ...
    for dim in dims:
        ...
        for alg in ap_conifg_list:
            sum_metric = 0
            for fun_id in fun_ids:
                df_ranking = pd.read_csv(pp_dir_path+'/sampling{}D/dim{}/f{}_DIM{}.csv'.format(sampling_multiplier, dim, fun_id, dim))
                if alg not in df_ranking['alg'].to_list():
                    metric_value = float(df_ranking[per_metric].to_list().max())
                    sum_metric += metric_value
                    break
                
                alg_num = int(df_ranking.index[df_ranking['alg'] == alg].values)
                metric_value = float(df_ranking.at[alg_num, per_metric])
                sum_metric += metric_value
...
def pp_alg_portfolio_data_loio_cv(bbob_suite, dims, feval_time, fun_ids, sampling_multiplier, target='-2.0', per_metric='accuracy'):
    """
    ...
    # # 1. Copy all the files that contain the number of function evaluations to reach the target value
    # copy_dir_path = os.path.join(ap_dir_path, 'accuracy_{}D')
    # os.makedirs(copy_dir_path, exist_ok=True)    
    # for alg in ap_conifg_list:
    #     for dim in dims:
    #         for fun_id in fun_ids:                
    ...
    #             original_file_path = './pp_{}_exdata/accuracy_{}D/{}/f{}_DIM{}.csv'.format(bbob_suite, param, alg, fun_id, dim)                
    #             with open(original_file_path, 'r') as fh:
    #                 for str_line in fh:
    #                     instance_id, feval, error, RMSE = str_line.split(',')
    #                     feval = int(float(feval.replace('\n','')))
    #                     error = int(float(error.replace('\n','')))
    #                     RMSE = int(float(RMSE.replace('\n','')))
    #                     feval_copy_file_path = os.path.join(copy_dir_path, '{}_f{}_DIM{}_i{}.csv'.format(alg, fun_id, dim, instance_id))
    #                     with open(feval_copy_file_path, 'w') as fh:
    #                         fh.write("{},{}".format(feval, error, RMSE))
    #                     break

    # # 2. Copy the files for the calculation of a given metric (error or RMSE)
    # copy_dir_path = os.path.join(ap_dir_path, per_metric)
    # os.makedirs(copy_dir_path, exist_ok=True)
    # for alg in ap_conifg_list:
    #     for dim in dims:
    ...
    # copy_dir_path = os.path.join(ap_dir_path, '{}D'.format(feval_time))
    metric_dir_path = os.path.join(ap_dir_path, per_metric)
    # os.makedirs(copy_dir_path, exist_ok=True)
    os.makedirs(metric_dir_path, exist_ok=True)
    for alg in ap_conifg_list:
        for dim in dims:
            for fun_id in fun_ids:
                    ...
                    with open(original_file_path, 'r') as fh:
                        for str_line in fh:
                            ...
                            if feval <= feval_time*dim:
                                ...
                                #     elif per_metric == 'RMSE':
                                #         metric_value = RMSE
                                #     metric_copy_file_path = os.path.join(metric_dir_path, '{}_f{}_DIM{}_liid{}.csv'.format(alg, fun_id, dim, instance_id))
                                #     with open(metric_copy_file_path, 'a') as fh:
                                #         fh.write("{},{}\n".format(iid, metric_value))
                ...
                for left_instance_id in range(0, 5+1):
                    metric_copy_file_path = os.path.join(metric_dir_path, '{}_f{}_DIM{}_liid{}.csv'.format(alg, fun_id, dim, left_instance_id))
                    with open(metric_copy_file_path, 'w') as fh:
                        for i, value in enumerate(metric_value):
                            if left_instance_id != i+1:
                                fh.write("{},{}\n".format(i+1, value))
...
def ecdf_vbs(bbob_suite='bbob', vbs_dir_path='./', all_fun_ids=range(1,24+1), test_instance_ids=[1, 2, 3, 4, 5], dim=2, target='-2', ecdf_file_path='./'):
    ...
    for fun_id in all_fun_ids:
        ...
        with open(vbs_file_path, 'r') as fh:
            l_strip = [s.rstrip() for s in fh.readlines()]
            vbs_alg = l_strip[0]
        
        # 改変途中。意図不明なためrun()の該当箇所はコメントアウト済み。
                               
        fevals_file_path = os.path.join('pp_bbob_exdata/accuracy{}D'.format(param), vbs_alg, 'f{}_DIM{}.csv'.format(fun_id, dim))
...
def vbs_construction(ap_dir_path, dim, feval_time, sampling_multiplier, fun_id, vbs_file_path, per_metric, sub_metric):
    ap_config_file_path = os.path.join(ap_dir_path, 'ap_config.csv')
    ...
    for alg in ap_algs:
        ...
        alg_df = df_ranking[df_ranking['alg'] == alg]
        score = int(alg_df[per_metric].values)
        # sub_metric_score = float(alg_df[sub_metric].values)
        if score < best_score:
            vbs_list =[]
            best_score = score
            # best_alg = alg
            # best_sub_metric = sub_metric_score
            vbs_list.append(alg)
        elif score == best_score:
            # if sub_metric_score < best_sub_metric:
            #     best_alg = alg
            #     best_sub_metric = sub_metric_score
            vbs_list.append(alg)
...
def run():
    ...
    for feval_time in [100]:
        ...
        portfolio_construction(portfolio_list, sampling)
        ...
        for sampling_multiplier in sampling:             
            ...
            for per_metric in ['accuracy']:
                # # # for LOIO-CV
                pp_alg_portfolio_data_loio_cv(bbob_suite, dims, feval_time, all_fun_ids, sampling_multiplier, target='-2.0', per_metric=per_metric)
                # # # for LOPO-CV and LOPOAD-CV
                pp_alg_portfolio_data(bbob_suite, dims, feval_time, all_fun_ids, sampling_multiplier, target='-2.0', per_metric=per_metric)
                ...
                for dim in dims:
                    for fun_id in all_fun_ids:
                        vbs_file_path = os.path.join(vbs_dir_path, 'f{}_DIM{}.csv'.format(fun_id, dim))
                        vbs_construction(ap_dir_path, dim, feval_time, sampling_multiplier, fun_id, vbs_file_path, per_metric, sub_metric)

            # ecdfの意図が不明なためコメントアウト。
